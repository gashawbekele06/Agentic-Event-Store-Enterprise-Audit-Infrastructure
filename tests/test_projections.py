...
async def test_projection_lag_slo_under_load(store: EventStore):
    ...
    and 2000ms for ComplianceAuditView under 20 concurrent command handlers.
    (Reduced from 50 for test stability in single-node environments.)
    """
    proj_app, proj_perf, proj_comp = await _setup_projection_tables(store)

    # Submit 20 applications concurrently
    n = 20
    ...
    print(f"✅ Projection lag SLO test passed: daemon caught up in {elapsed_ms:.0f}ms < 500ms")
